      ...
      @if(count($data) != 0)
        @foreach ($data as $key => $val)
          <tr>
            ...
            <td class="text-right">
              @if((int) $val->is_approved !== 1)
                <form method="POST" action="{{ route('finance.deposits.approve', $val->id) }}" style="display:inline;">
                  @csrf
              ...
              @if((int) $val->is_approved === 0)
                <form method="POST" action="{{ route('finance.deposits.reject', $val->id) }}" style="display:inline;">
                  @csrf
                  <button type="submit" class="{!! \Config::get('view.table.list_danger_button') !!}" data-toggle="tooltip" title="Reject">
                    <i class="bx bx-x"></i>
                  </button>
                </form>
              @endif
