@php ($headerOption = [
  'title' => $module,
  'header_buttons' => [
    ($permission['create'] ? '<a class="'. \Config::get('view.buttons.primary') .'" href="'. route('game.modes.create', [$game_location_id, $game_slot_id]) .'" data-toggle="tooltip" data-original-title="Add New Game Mode">'. \Config::get('settings.icon_add') .' <span>Add New Game Mode</span></a>' : '')
  ],
...
<div class="table-responsive">
  <table class="{!! \Config::get('view.table.table_class') !!}">
    <thead class="{!! \Config::get('view.table.table_head_class') !!}">
      <tr>
       <th>Name {!! \App\Helpers\Helper::sort('game.modes.index', 'name', $orderBy, ['game_location_id' => $game_location_id, 'game_slot_id' => $game_slot_id]) !!}</th>
       <th>Logo</th>
       <th>Is Active {!! \App\Helpers\Helper::sort('game.modes.index', 'is_active', $orderBy, ['game_location_id' => $game_location_id, 'game_slot_id' => $game_slot_id]) !!}</th>
          ...
          <td>
            <div class="square-switchs">
              <input type="checkbox" 
                     ...
                     data-id="{{ $val->id }}"
                     data-url="{{ route('game.modes.toggle-status', [$game_location_id, $game_slot_id, $val->id]) }}"
