...
class GameMode extends Model
{
    ...
    public function getFilters($routeName = 'game.modes.index', $game_location_id = null, $game_slot_id = null) {
        if ($game_location_id !== null && $game_slot_id !== null) {
            $resetRoute = route($routeName, ['game_location_id' => $game_location_id, 'game_slot_id' => $game_slot_id]);
        } else {
    ...
    public function getListing($srch_params = [], $offset = 0) {
        try {
            ...
            $listing = self::select($select)
                ...
                ->when(isset($srch_params['slot_id']), function ($q) use ($srch_params) {
                    return $q->where($this->table . '.slot_id', '=', $srch_params['slot_id']);
                })
    ...
    public function store($input = [], $id = 0, $request = null) {
        try {
            ...
            if ($request && $request->hasFile('logo')) {
                $file = \App\Models\File::upload($request, 'logo', 'game_mode_logo', $data->id);
